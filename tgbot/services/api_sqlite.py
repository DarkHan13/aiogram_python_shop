...
# ГОРОДА
# Добавление города
def add_townx(town_name):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO storage_town (town_id, town_name) VALUES (?, ?)",
                    [random.randint(1000000000, 9999999999), town_name])
        con.commit()


# Удаление города
def delete_townx(town_id):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = f"delete FROM storage_town WHERE town_id = {town_id}"
        con.execute(sql)
        con.execute(f"DELETE FROM storage_item WHERE town_id = {town_id} ")
        con.commit()


# Получение города
def get_townx(**kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = f"SELECT * FROM storage_town"
        sql, parameters = update_format_args(sql, kwargs)
        return con.execute(sql, parameters).fetchone()


# Получение городов
def get_townsx(**kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = f"SELECT * FROM storage_town"
        sql, parameters = update_format_args(sql, kwargs)
...
def clear_townsx():
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "DELETE FROM storage_town"
        con.execute(sql)
        con.commit()


# Удаление города
def remove_townx(**kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "DELETE FROM storage_town"
        sql, parameters = update_format_args(sql, kwargs)
...
# Добавление товара
def add_itemx(category_id, position_id, get_all_items, user_id, user_name, town_id):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        ...
        for item_data in get_all_items:
            if not item_data.isspace() and item_data != "":
                con.execute("INSERT INTO storage_item "
                            "(item_id, item_data, position_id, category_id, creator_id, creator_name, add_date, town_id) "
                            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                            [random.randint(1000000000, 9999999999), clear_html(item_data.strip()), position_id, category_id,
                             user_id, user_name, get_date(), town_id])
        con.commit()
...
# Добавить промокод
def add_promocodex(discount, promocode):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO storage_promocode (code, discount) VALUES (?, ?)",
                    [promocode, discount])
        con.commit()


# Удалить использованный промокод
def delete_promocodex(**kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "DELETE FROM storage_promocode"
        sql, parameters = update_format_args(sql, kwargs)
        sql_select = "SELECT * FROM storage_promocode"
        sql_select, parameters = update_format_args(sql_select, kwargs)
        rows = con.cursor().execute(sql_select, parameters).fetchall()
...
def create_dbx():
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        # Создание БД с хранением городов
        if len(con.execute("PRAGMA table_info(storage_town)").fetchall()) == 3:
            print("DB was found(9/10)")
        else:
            con.execute("CREATE TABLE storage_town("
                        "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                        "town_id INTEGER,"
                        "town_name TEXT)")
        ...
        if len(con.execute("PRAGMA table_info(storage_promocode)").fetchall()) == 3:
            print("DB was found(10/10)")
        else:
            con.execute("CREATE TABLE storage_promocode("
                        "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                        "discount INTEGER,"
                        "code TEXT UNIQUE )")
