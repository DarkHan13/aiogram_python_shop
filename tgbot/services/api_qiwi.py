    ...
    async def bill_pay(self, get_amount, get_way):
        ...
        if response:
            ...
            if get_way == "Form":
                ...
                bill = myBill(amount=get_amount, bill_id=receipt, comment=receipt, auth_key=self.secret)
                ...
                send_requests = f"https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={self.login}&amountInteger=" \
                                f"{get_amount}&amountFraction=0&extra%5B%27comment%27%5D={receipt}&currency=KZT" \
