...
# Кнопка сгенерировать промокод
@dp.message_handler(IsAdmin(), text="🈹 Промокод", state="*")
async def admin_products(message: Message, state: FSMContext):
    await state.finish()

    await state.set_state("generate_promo_code")
    await message.answer("<b>🎁 Сгенерировать промокод.\nВведите скидку (целое число от 1 до 100): </b>")
