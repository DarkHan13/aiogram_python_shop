...
# Открытие товаров
@dp.callback_query_handler(text_startswith="buy_town_open", state="*")
async def user_shop(call: CallbackQuery, state: FSMContext):
    town_id = int(call.data.split(":")[1])

    get_town = get_categoryx(category_id=town_id)

    if len(get_all_categoriesx()) >= 1:
        await call.message.edit_text("<b>🎁 Выберите нужный вам товар:</b>",
                             reply_markup=products_item_category_open_fp(0, town_id))
    else:
        await call.answer("<b>🎁 Товары в данное время отсутствуют.</b>")


# Открытие городов
@dp.message_handler(text="🎁 Купить", state="*")
@dp.callback_query_handler(text="return_to_towns", state="*")
async def user_town(message: CallbackQuery, state: FSMContext):
    await state.finish()

    if len(get_all_townsx()) >= 1:
        if isinstance(message, Message):
            await message.answer("<b>🎁 Выберите нужный вам город:</b>",
                                 reply_markup=products_item_town_open_fp())
        else:
            await message.message.edit_text("<b>🎁 Выберите нужный вам город:</b>",
                                 reply_markup=products_item_town_open_fp())
    else:
        await message.answer("<b>🎁 Города в данное время отсутствуют.</b>")
...
async def user_purchase_category_open(call: CallbackQuery, state: FSMContext):
    ...
    town_id = int(call.data.split(":")[2])
    get_category = get_categoryx(category_id=category_id)
    get_positions = get_positionsx(category_id=category_id)

    if len(get_positions) >= 1:
        await call.message.edit_text("<b>🎁 Выберите нужный вам товар:</b>",
                                     reply_markup=products_item_position_open_fp(0, category_id, town_id))
    else:
...
async def user_purchase_category_return(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_categories) >= 1:
        await call.message.edit_text("<b>🎁 Выберите нужный вам товар:</b>",
                                     reply_markup=products_item_category_open_fp(0, town_id=town_id))
    else:
...
async def user_purchase_category_next_page(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>🎁 Выберите нужный вам товар:</b>",
                                 reply_markup=products_item_category_next_page_fp(remover, town_id=town_id))
...
async def user_purchase_position_open(call: CallbackQuery, state: FSMContext):
    ...
    town_id = int(call.data.split(":")[4])
    town_name = get_townsx(town_id=town_id)[0]["town_name"]

    get_position = get_positionx(position_id=position_id)
    get_category = get_categoryx(category_id=category_id)
    get_items = get_itemsx(position_id=position_id, town_id=town_id)
    ...
    if len(get_position['position_photo']) >= 5:
        ...
        await call.message.answer_photo(get_position['position_photo'],
                                        send_msg, reply_markup=products_open_finl(position_id, remover, category_id, town_id=town_id))
    else:
        await call.message.edit_text(send_msg,
                                     reply_markup=products_open_finl(position_id, remover, category_id, town_id=town_id))
...
async def user_purchase_position_return(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        ...
        await call.message.answer("<b>🎁 Выберите нужный вам товар:</b>",
                                  reply_markup=products_item_position_open_fp(remover, category_id, town_id=town_id))
    else:
        await call.message.edit_text("<b>🎁 Товары в данное время отсутствуют.</b>")
...
async def user_purchase_position_next_page(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>🎁 Выберите нужный вам товар:</b>",
                                 reply_markup=products_item_position_next_page_fp(remover, category_id, town_id=town_id))
...
async def user_purchase_position_prev_page(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>🎁 Выберите нужный вам товар:</b>",
                                 reply_markup=buy_position_return_page_fp(remover, category_id, town_id=town_id))
...
async def user_purchase_select(call: CallbackQuery, state: FSMContext):
    position_id = int(call.data.split(":")[1])
    discount_str = call.data.split(":")[2]
    town_id = call.data.split(":")[3]
    discount = 0
    if discount_str != "None":
    ...
    price = int(get_position['position_price'] - get_position['position_price'] * (discount / 100))

    if get_position['position_price'] != 0 and get_user["user_balance"] != 0:
    ...
    if int(get_user['user_balance']) >= price:
        if get_count == 1 or discount_str != "None" and get_count >= 1:
            ...
            await state.update_data(price=get_position['position_price'])
...
async def user_purchase_confirm(call: CallbackQuery, state: FSMContext):
    ...
    if get_action == "yes":
        ...
        amount_pay = int(price * get_count)
        ...
        if 1 <= int(get_count) <= len(get_items):
            if int(get_user['user_balance']) >= amount_pay:
                ...
                if get_count != send_count:
                    amount_pay = int(price * send_count)
...
@dp.callback_query_handler(text_startswith="enter_promo_code", state="*")
async def enter_promo_code(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(town_id=town_id)
    await call.message.answer("<b>Введите промокод:</b>")

@dp.message_handler(state="enter_promo_code")
async def enter_promo_code_2(message: Message, state: FSMContext):
    result = delete_promocodex(code=message.text)
    text = "Промокод не действителен"
    if len(result) > 0:
        text = f"Скидка на товар: {result[0]['discount']}%"
        data = await state.get_data()
        position_id = data["position_id"]
        await message.answer(text, reply_markup=buy_item_select_promo_code(position_id,
