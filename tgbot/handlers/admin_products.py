...
from tgbot.keyboards.inline_z_all import category_remove_confirm_inl, position_remove_confirm_inl, \
    item_remove_confirm_inl, close_inl, town_remove_confirm
...
@dp.message_handler(IsAdmin(), text="🏬 Добавить Города ➕", state="*")
async def product_category_create(message: Message, state: FSMContext):
    await state.finish()

    await state.set_state("here_town_name")
    await message.answer("<b>🏬 Введите название города 🏷</b>")

# Кнопки с подтверждением удаления всех категорий
@dp.message_handler(IsAdmin(), text="🏬 Удалить все города ❌", state="*")
async def product_item_remove(message: Message, state: FSMContext):
    await state.finish()

    await message.answer("<b>🎁 Вы действительно хотите удалить все города? ❌</b>\n",
                         reply_markup=town_remove_confirm)

################################################################################################
####################################### СОЗДАНИЕ ГОРОДА #####################################
# Принятие названия категории для её создания
@dp.message_handler(IsAdmin(), state="here_town_name")
async def product_category_create_name(message: Message, state: FSMContext):
    if len(message.text) <= 100:
        add_townx(clear_html(message.text))

        await state.finish()
        await message.answer("<b>🏬 город был успешно добавлен ✅</b>")
    else:
...
async def product_item_create_start(message: Message, state: FSMContext):
    await state.finish()

    if len(get_all_townsx()) >= 1 and len(get_all_townsx()) >= 1:
        await message.answer("<b>🎁 Выберите категорию с нужной позицией</b>",
                             reply_markup=products_add_town_open_fp())
    else:
        await message.answer("<b>❌ Отсутствуют позиции или города для добавления товара.</b>")


# Начальные категории для добавления товаров
@dp.callback_query_handler(IsAdmin(), text_startswith="town_open", state="*")
async def product_item_create(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_all_positionsx()) >= 1:
        await call.message.edit_text("<b>🎁 Выберите категорию с нужной позицией</b>",
                             reply_markup=products_add_category_open_fp(0, town_id=town_id))
    else:
        await call.answer("<b>❌ Отсутствуют позиции для добавления товара.</b>")
...
async def product_item_load_category_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positionsx(category_id=category_id)) >= 1:
        await call.message.delete()
        await call.message.answer("<b>🎁 Выберите нужную вам позицию</b>",
                                  reply_markup=products_add_position_open_fp(0, category_id, town_id))
    else:
        await call.answer("🎁 Позиции в данной категории отсутствуют")
...
async def product_item_load_open(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(here_cache_add_item_town_id=town_id)
...
async def product_item_load_get(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        town_id = data['here_cache_add_item_town_id']
        data['here_count_add_items'] += count_add

    get_user = get_userx(user_id=message.from_user.id)
    add_itemx(category_id, position_id, get_all_items, get_user['user_id'], get_user['user_name'], town_id=town_id)
...
# Согласие на удаление всех городов
@dp.callback_query_handler(IsAdmin(), text_startswith="confirm_remove_town:", state="*")
async def product_item_remove(call: CallbackQuery, state: FSMContext):
    get_action = call.data.split(":")[1]

    if get_action == "yes":
        get_items = len(get_all_townsx())
        clear_townsx()

        await call.message.edit_text(f"<b>🎁 Вы удалили все города<code>({get_items}шт)</code> ☑</b>")
    else:
        await call.message.edit_text("<b>🎁 Вы отменили удаление всех городов ✅</b>")


# Меню удаления городов
@dp.message_handler(IsAdmin(), text="🏬 Удалить город 🖍", state="*")
async def product_category_create(message: Message, state: FSMContext):
    await state.finish()

    if len(get_all_townsx()) >= 1:
        await message.answer("<b>🎁 Выберите город, который вы хотите удалить:</b>",
                             reply_markup=town_delete())
    else:
        await message.answer("<b>🎁 Города в данное время отсутствуют.</b>")


# Удалить город
@dp.callback_query_handler(IsAdmin(), text_startswith="delete_town:", state="*")
async def product_item_remove(call: CallbackQuery, state: FSMContext):
    town_id = int(call.data.split(":")[1])
    town_name = get_townsx(town_id=town_id)[0]["town_name"]
    delete_townx(town_id)

    await call.message.edit_text(f"<b>🎁 Вы удалили город <code>\"{town_name}\"</code> ☑</b>")
