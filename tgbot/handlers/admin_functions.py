...
@dp.message_handler(IsAdmin(), state="generate_promo_code")
async def generate_promo_code(message: Message, state: FSMContext):
    await state.finish()
    discount = message.text
    try:
        int(discount)
    except ValueError:
        await message.answer("<b>Нужно целое число</b>")
    if not 1 <= int(discount) <= 100:
        await message.answer("Число не в диапазоне от 1 до 100")
        return
    promo_code = generate_random_string(8)
    add_promocodex(int(discount), promocode=promo_code)
    await message.answer(
        f"<b>Промокод на скидку {discount}%\n</b>"
