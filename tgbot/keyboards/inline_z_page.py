...
from tgbot.services.api_sqlite import get_all_categoriesx, get_itemsx, get_positionsx, get_all_townsx, get_townsx
...
def products_add_category_open_fp(remover, town_id):
    get_categories = get_all_categoriesx()
    keyboard = InlineKeyboardMarkup()
    count = 0
    ...
    for a in range(remover, len(get_categories)):
        if count < cpage:
            keyboard.add(ikb(f"{get_categories[a]['category_name']}",
                             callback_data=f"products_add_category:{get_categories[a]['category_id']}:{town_id}"))
        count += 1
...
# TOWN TOWN
def products_add_town_open_fp():
    get_town = get_all_townsx()
    keyboard = InlineKeyboardMarkup()
    for a in range(len(get_town)):
        keyboard.add(ikb(f"{get_town[a]['town_name']}",
                         callback_data=f"town_open:{get_town[a]['town_id']}"))
    return keyboard
...
def products_add_position_open_fp(remover, category_id, town_id):
    get_positions = get_positionsx(category_id=category_id)
    keyboard = InlineKeyboardMarkup()
    count = 0
    ...
    keyboard.add(ikb("⬅ Вернуться ↩", callback_data=f"back_add_products_to_category:{town_id}"))
...
def products_item_town_open_fp():
    get_town = get_all_townsx()
    keyboard = InlineKeyboardMarkup()
    for a in range(len(get_town)):
        keyboard.add(ikb(f"{get_town[a]['town_name']}",
                         callback_data=f"buy_town_open:{get_town[a]['town_id']}"))
    return keyboard


# Стартовые страницы категорий при покупке товара
def products_item_category_open_fp(remover, town_id):
    get_categories = get_all_categoriesx()
    keyboard = InlineKeyboardMarkup()
    count = 0
    ...
    for a in range(remover, len(get_categories)):
        if count < cpage:
            keyboard.add(ikb(f"{get_categories[a]['category_name']}",
                             callback_data=f"buy_category_open:{get_categories[a]['category_id']}:{town_id}"))
        count += 1
    ...
    if len(get_categories) <= 10:
        ...
        keyboard.add(
            ikb("🔸 1 🔸", callback_data="..."),
            ikb("Далее ➡", callback_data=f"buy_category_nextp:{remover + cpage}:{town_id}")
        )
    elif remover + cpage >= len(get_categories):
        keyboard.add(
            ikb("⬅ Назад", callback_data=f"buy_category_backp:{remover - cpage}:{town_id}"),
            ikb(f"🔸 {str(remover + cpage)[:-1]} 🔸", callback_data="...")
        )
    else:
        keyboard.add(
    ...
    keyboard.add(ikb(f"⬅ Вернуться ↩", callback_data="return_to_towns"))
    return keyboard


# Следующая страница категорий при покупке товара
def products_item_category_next_page_fp(remover, town_id):
    get_categories = get_all_categoriesx()
    keyboard = InlineKeyboardMarkup()
    count = 0
...
def products_item_category_back_page_fp(remover, town_id):
    get_categories = get_all_categoriesx()
    keyboard = InlineKeyboardMarkup()
    count = 0
...
def products_item_position_open_fp(remover, category_id, town_id):
    get_positions = get_positionsx(category_id=category_id)
    get_town = get_townsx(town_id=town_id)
    town_name = get_town[0]["town_name"]
    keyboard = InlineKeyboardMarkup()
    count = 0
    print("GOD")
    for a in range(remover, len(get_positions)):
    ...
    if len(get_positions) <= 10:
        ...
        keyboard.add(
            ikb("🔸 1 🔸", callback_data="..."),
            ikb("Далее ➡", callback_data=f"buy_position_nextp:{remover + cpage}:{category_id}:{town_id}")
        )
    elif remover + cpage >= len(get_positions):
        keyboard.add(
    ...
    keyboard.add(ikb("⬅ Вернуться ↩", callback_data=f"buy_category_return:{town_id}"))

    return keyboard


# Следующая страница позиций для покупки товаров
def products_item_position_next_page_fp(remover, category_id, town_id):
    ...
    if remover + cpage >= len(get_positions):
        keyboard.add(
            ikb("⬅ Назад", callback_data=f"buy_position_backp:{remover - cpage}:{category_id}:{town_id}"),
            ikb(f"🔸 {str(remover + cpage)[:-1]} 🔸", callback_data="...")
        )
    else:
        keyboard.add(
...
def buy_position_return_page_fp(remover, category_id, town_id):
    get_positions = get_positionsx(category_id=category_id)
    keyboard = InlineKeyboardMarkup()
    count = 0
...
def town_delete():
    ...
    for a in range(len(get_town)):
        keyboard.add(ikb(f"{get_town[a]['town_name']}",
                         callback_data=f"delete_town:{get_town[a]['town_id']}"))
    return keyboard
